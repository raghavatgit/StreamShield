/! Enumerates all visible top-level windows and extracts process info.
...
pub fn enumerate_windows() -> Vec<WindowInfo> {
    ...
    unsafe {
        EnumWindows(Some(enum_proc), &mut hwnds as *mut Vec<HWND> as LPARAM);
    }

    let mut results: Vec<WindowInfo> = Vec::new();
    let mut seen_exe: HashSet<String> = HashSet::new();
    let mut pid_cache: HashMap<u32, String> = HashMap::new();
    ...
    for hwnd in hwnds {
        ...
        if exe_name == "streamshield.exe" {
            continue;
        }

        if seen_exe.contains(&exe_name) {
            continue;
        }
